...
static void example_task(void *args)
{
    ...
    */
    ...
    int freeRAM = heap_caps_get_largest_free_block(MALLOC_CAP_8BIT);
    ESP_LOGI(TAG, "Total free RAM = %d", freeRAM);   
    printf("\n\n");
    ...
    while (1) {
        ...
        ESP_LOGI(TAG, "Memory Successfully Created at %d", (int) ptr);
        esp_dump_per_task_heap_info();
        free(ptr);
        ESP_LOGI(TAG, "Memory Successfully Freed at %d", (int) ptr);
        printf("\n\n");
