...
esp_err_t api_scan_handler(httpd_req_t *req)
{
    ...
    cJSON *scanlist = cJSON_CreateArray();
    cJSON_AddItemToObject(scan, "scanlist", scanlist);

    for (int i = 0; i < ap_count; i++)
    {
        ap = cJSON_CreateString( (const char*) ap_info[i].ssid);
        cJSON_AddItemToArray(scanlist, ap);
    }

    string = cJSON_Print(scan);
    printf("JSON Scan List:\n%s\n", string);
    cJSON_Delete(scan);
    response = httpd_resp_send(req, string, HTTPD_RESP_USE_STRLEN);
